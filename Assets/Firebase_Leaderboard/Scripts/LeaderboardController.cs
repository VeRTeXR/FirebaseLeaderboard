...
namespace Firebase_Leaderboard.Scripts {
  public class LeaderboardController : MonoBehaviour {
    ...
    private void Update() {
      if (!isInitialized) {
        if (readyToInitialize) {
          ...
          Debug.LogError("Database Instance Reference : "+ dbref);
          
          isInitialized = true;
          refreshScores = true;
          readyToInitialize = false;
          if (FirebaseInitialized != null) FirebaseInitialized(this, null);
        }
        return;
      }
      if (refreshScores) {
    ...
    public void GetUserScore(string userId) {
      if (!isInitialized && !getUserScoreCallQueued) {
        Debug.LogWarning(
    ...
    private void GetInitialTopScores(long batchEnd) {
      ...
      var startTimeSpan = StartDate.Ticks / TimeSpan.TicksPerSecond;
      var endTimeSpan = EndDate.Ticks / TimeSpan.TicksPerSecond;
      var dbQuery = dbref.Child(AllScoreDataPath).OrderByChild("score");
      if (LowestFirst) {
        dbQuery = dbQuery.StartAt(batchEnd).LimitToFirst(ScoresToRetrieve);
      } else {
        dbQuery = dbQuery.EndAt(batchEnd).LimitToLast(ScoresToRetrieve);
      }
      dbQuery.GetValueAsync().ContinueWith(task => {
        if (task.Exception != null) {
          SetTopScores();
          return;
        ...
        var scores = ParseValidUserScoreRecords(task.Result, startTimeSpan, endTimeSpan);
